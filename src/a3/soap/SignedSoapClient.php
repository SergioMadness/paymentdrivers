...
namespace professionalweb\paymentdrivers\a3\soap;
...
class SignedSoapClient extends \SoapClient
{
    ...
    {
        if ($data === null) $data = microtime().uniqid();
        $id   = md5($data);
        return sprintf('%08s-%04s-%04s-%04s-%012s', substr($id, 0, 8),
            substr($id, 8, 4), substr($id, 12, 4), substr(16, 4),
            substr($id, 20));
    }
    ...
    function canonicalizeXML($data)
    {
        ...
        $f    = popen(sprintf('%s --exc-c14n %s', self::XMLLINT_PATH, $fname),
            'r');
    ...
    function buildSignedInfo($dom, $ids)
    {
        ...
        $method     = $signedInfo->appendChild($dom->createElementNS(self::DS_NS,
                'ds:CanonicalizationMethod'));
        $method->setAttribute('Algorithm',
        ...
        foreach ($ids as $id) {
            ...
            $reference  = $signedInfo->appendChild($dom->createElementNS(self::DS_NS,
                    'ds:Reference'));
            $reference->setAttribute('URI', "#{$id}");
            $transforms = $reference->appendChild($dom->createElementNS(self::DS_NS,
                    'ds:Transforms'));
            $transform  = $transforms->appendChild($dom->createElementNS(self::DS_NS,
                    'ds:Transform'));
            // укажем, что провели канонизацию
            $transform->setAttribute('Algorithm',
                'http://www.w3.org/2001/10/xml-exc-c14n#');
            // и сделаем дайджест с помощью SHA1
            $method     = $reference->appendChild($dom->createElementNS(self::DS_NS,
                    'ds:DigestMethod'));
            $method->setAttribute('Algorithm',
                'http://www.w3.org/2000/09/xmldsig#sha1');
            $reference->appendChild($dom->createElementNS(self::DS_NS,
                    'ds:DigestValue', base64_encode(sha1($canonicalized, true))));
        }
    ...
    function buildSecurityToken($dom, $cert, $certpasswd, &$pkeyid, &$tokenId)
    {
        ...
        $token = $dom->createElementNS(self::WSSE_NS,
            'wsse:BinarySecurityToken', $pubcert);
    ...
    function __doRequest($request, $location, $action, $version, $one_way = 0)
    {
        ...
        if (!$headernode)
                $headernode = $dom->documentElement->insertBefore($dom->createElementNS(self::SOAP_NS,
                    'SOAP-ENV:Header'), $bodynode);
        ...
        $secNode = $headernode->appendChild($dom->createElementNS(self::WSSE_NS,
                'wsse:Security'));

        // добавляем ссылку на ключ
        $secNode->appendChild($this->buildSecurityToken($dom,
                $this->_ssl_options['cert'],
                empty($this->_ssl_options['certpasswd']) ? '' : $this->_ssl_options['certpasswd'],
                $pkeyid, $tokenId));

        // ссылка на подписываемый элемент
        $signNode = $secNode->appendChild($dom->createElementNS(self::DS_NS,
                'ds:Signature'));

        $tokenNode = $secNode->appendChild($dom->createElementNS(self::WSSE_NS,
                'wsse:UsernameToken'));
        $tokenNode->setAttributeNS(self::WSU_NS, 'wsu:Id', 'usernameToken');

        $userNameNode = $tokenNode->appendChild($dom->createElementNS(self::WSSE_NS,
                'wsse:Username', '987564321'));
        $tokenNode->appendChild($dom->createElementNS(self::WSSE_NS,
                'wsse:Password'));
        ...
        $signNode->appendChild($dom->createElementNS(self::DS_NS,
                'ds:SignatureValue', base64_encode($signature)));
        $keyInfo           = $signNode->appendChild($dom->createElementNS(self::DS_NS,
                'ds:KeyInfo'));
        $secTokRef         = $keyInfo->appendChild($dom->createElementNS(self::WSSE_NS,
                'wsse:SecurityTokenReference'));
        $keyRef            = $secTokRef->appendChild($dom->createElementNS(self::WSSE_NS,
                'wsse:KeyIdentifier'));
        $keyRef->setAttribute('EncodingType',
            "http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-soap-message-security-1.0#Base64Binary");
        $keyRef->setAttribute('ValueType',
            "http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-x509-token-profile-1.0#X509SubjectKeyIdentifier");
        $keyRef->nodeValue = 'Ys+Sd8wW9q6OY1ZFyd9R46fSkZ4=';
